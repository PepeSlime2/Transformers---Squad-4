...
Otimizado para performance e organização.
...
# Configuração MLflow centralizada
def setup_mlflow():
...
class ModelLogger:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        # Log de todos os parâmetros e métricas acumulados
...
# Funções de benchmark padronizadas
...
def benchmark_generation_model(model_name: str, display_name: str, 
    ...
    with ModelLogger(f"{display_name}_{task_type}") as logger:
        ...
        metrics = MetricsCalculator.compute_generation_metrics(predictions, prompts)
    ...
    if results:
        ...
        for task in df['task'].unique():
            ...
            if task == 'classification':
                ...
                print(f"{'Modelo':<20} {'BLEU':<10} {'ROUGE-L':<10} {'Inferência(s)':<15} {'Quantizado':<12}")
                for _, row in task_df.iterrows():
                    print(f"{row['model']:<20} {row.get('bleu', 0):<10.4f} "
                          f"{row.get('rougeL', 0):<10.4f} {row.get('inference_time_seconds', 0):<15.2f} "
