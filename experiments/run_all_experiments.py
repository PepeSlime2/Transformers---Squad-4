#!/usr/bin/env python
"""
Executa os scripts de experimento individual em sequência para evitar problemas de OOM
ou conflitos de download. Usa subprocess para manter processos isolados e gerar logs.
"""
import subprocess
import sys
import os
...
if __name__ == '__main__':
    ...
    for script in EXPERIMENT_SCRIPTS:
        ...
        if ret != 0:
            print(f"⚠️ O script {script} retornou erro (code={ret}). Confira logs e mlflow/outputs.")
        print(f"⏱️ Aguardando {SLEEP_BETWEEN}s antes de iniciar o próximo...")
        sleep(SLEEP_BETWEEN)

    print('\n✅ Execução sequencial concluída. Veja os runs no MLflow ou CSVs gerados.')
